...
int main(int argc, char** argv)
{
    ...
    // Load arm actions from config
    ArmActionPlayer arm_player;
    arm_player.load(param::config["ArmActions"]);
    ...
    if (arm_player.has_actions()) {
        std::cout << "  --- Arm Actions (Velocity mode) ---\n";
        std::cout << arm_player.help_string();
        std::cout << "  0: stop arm action\n";
    }
    std::cout << "  x: exit\n";
    ...
    while (true)
    {
        ...
        // Overlay arm joint commands on top of policy output.
        // The 15-DOF policy only writes legs+waist (SDK 0-14);
        // arm joints (SDK 15-28) are untouched by run(), safe to overlay.
        if (arm_player.is_active() && current_state->isState(velocity->getState())) {
            arm_player.update(0.001f); // control_dt = 1ms
            arm_player.apply(FSMState::lowcmd->msg_.motor_cmd());
        }
