"""15-DOF rotation-enhanced config V2a — conservative fix + LCP smoothness.

Key fixes vs velocity_env_cfg_rot.py:
  1. Standing: 10% standing envs + stand_still penalty + feet contact reward
     → Solves: constant stepping even when no command
  2. Rotation distribution: 8% rotating envs (down from 20%)
     → Solves: stepping behavior bleeding into standing/walking
  3. Backward lean: backward_lean_penalty + lower base height target (0.76)
     → Solves: body leans backward
  4. Leg kick: stronger action_rate/joint_acc + LCP runner
     → Solves: jerky actions and backward leg kicks at low speed
  5. Rotation toned down: single_support 0.5, track_ang_vel_z 0.75
     → Prevents: over-aggressive rotation stepping
"""
...
# ---------------------------------------------------------------------------
# Fix 2: More standing practice + guide-compliant rotating fraction
# ---------------------------------------------------------------------------
@configclass
class CommandsCfg:
    base_velocity = mdp.UniformLevelVelocityCommandCfg(
        ...
        rel_standing_envs=0.10,       # ↑ from 0.05: more standing practice
        rel_rotating_envs=0.08,       # ↓ from 0.20: guide-compliant
...
class RewardsCfg:
    ...
    # --- Fix 4: Stronger smoothness penalties (complementing LCP) ---
    joint_vel = RewTerm(func=mdp.joint_vel_l2, weight=-0.001, params={"asset_cfg": SceneEntityCfg("robot", joint_names=TRAIN_JOINT_NAMES)})
    joint_acc = RewTerm(func=mdp.joint_acc_l2, weight=-5e-7, params={"asset_cfg": SceneEntityCfg("robot", joint_names=TRAIN_JOINT_NAMES)})
    ...
    # --- Fix 3: Posture — penalize backward lean + lower height target ---
    ...
    # --- Fix 1: Standing behavior ---
    stand_still = RewTerm(func=mdp.stand_still, weight=-0.5, params={"command_name": "base_velocity"})
    feet_contact_without_cmd = RewTerm(func=mdp.feet_contact_without_cmd, weight=0.5, params={"command_name": "base_velocity", "sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*ankle_roll.*")})

    # --- Rotation-aware yaw tracking (fix 5: toned down) ---
    ...
    # --- Pure rotation shaping (fix 5: reduced single support incentive) ---
