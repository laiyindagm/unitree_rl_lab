"""15-DOF velocity env with rotation-enhancement improvements.

Based on velocity_env_cfg.py with the following changes from the rotation
transfer guide:

Phase 1 - Command distribution & ang vel range:
  - rel_rotating_envs = 0.08 (8% pure-rotation samples)
  - rel_standing_envs = 0.05
  - initial ang_vel_z = (-0.2, 0.2)
  - limit ang_vel_z widened to (-1.0, 1.0)

Phase 2 - Straight-walk parasitic yaw suppression:
  - track_ang_vel_z replaced by rotation-aware variant
  - yaw_rate_l1 penalty for straight-walk only

Phase 3 - Pure rotation shaping:
  - single support reward
  - double support slide penalty
  - twist joint penalty

Stability:
  - PPO actor std already uses log-std in BasePPORunnerCfg (no change needed)
  - ang_vel_cmd_levels curriculum added
"""
...
# ---------------------------------------------------------------------------
# Guide Phase 1: command distribution with pure-rotation samples
# ---------------------------------------------------------------------------
@configclass
class CommandsCfg:
    base_velocity = mdp.UniformLevelVelocityCommandCfg(
        ...
        rel_standing_envs=0.05,       # guide: keep small standing fraction
        rel_rotating_envs=0.20,       # guide: 8% pure rotation envs
        ...
        ranges=mdp.UniformLevelVelocityCommandCfg.Ranges(
            lin_vel_x=(-0.1, 0.1),
            lin_vel_y=(-0.1, 0.1),
            ang_vel_z=(-0.2, 0.2),    # guide: initial ang_vel_z in learnable range
        ),
        limit_ranges=mdp.UniformLevelVelocityCommandCfg.Ranges(
            lin_vel_x=(-0.5, 1.5),
            lin_vel_y=(-0.5, 0.5),
            ang_vel_z=(-0.6, 0.6),    # guide: wider limit for curriculum growth
        ),
    )
...
@configclass
class RewardsCfg:
    # --- Original rewards (guide section V: do not remove) ---
    ...
    joint_acc = RewTerm(func=mdp.joint_acc_l2, weight=-2.5e-7, params={"asset_cfg": SceneEntityCfg("robot", joint_names=TRAIN_JOINT_NAMES)})
    ...
    # --- Guide Phase 2: rotation-aware yaw tracking ---
    # Replace the old track_ang_vel_z with rotation-aware variant
    track_ang_vel_z = RewTerm(
        func=mdp.track_ang_vel_z_rotating_aware,
        weight=0.5 * 2,
        params={"command_name": "base_velocity", "std": math.sqrt(0.25)},
    )
    # Parasitic yaw penalty: only for straight-walk, weak weight
    yaw_rate_l1 = RewTerm(
    ...
    # --- Guide Phase 3: pure rotation shaping ---
...
# ---------------------------------------------------------------------------
# Guide Phase 1 (cont.): ang_vel curriculum
# ---------------------------------------------------------------------------
@configclass
class CurriculumCfg:
